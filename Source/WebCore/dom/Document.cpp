...
void Document::flushAutofocusCandidates()
{
    ...
    // If topDocument's focused area is not topDocument itself, empty
    // candidates, set autofocus processed, and return.
    if (focusedElement() || page->focusController().focusedFrame() != frame()) {
        m_autofocusCandidates.clear();
