...
{
    if (auto* svgShape = dynamicDowncast<LegacyRenderSVGShape>(object))
        return svgShape && !svgShape->isRenderingDisabled();

    if (auto* text = dynamicDowncast<RenderSVGText>(object))
        return text && text->isObjectBoundingBoxValid();

    if (auto* hiddenContainer = dynamicDowncast<LegacyRenderSVGContainer>(object))
        return hiddenContainer && !hiddenContainer->isLegacyRenderSVGHiddenContainer();

    if (auto* foreignObject = dynamicDowncast<LegacyRenderSVGForeignObject>(object))
        return foreignObject && foreignObject->isObjectBoundingBoxValid();

    if (auto* svgImage = dynamicDowncast<LegacyRenderSVGImage>(object))
        return svgImage && svgImage->isObjectBoundingBoxValid();
